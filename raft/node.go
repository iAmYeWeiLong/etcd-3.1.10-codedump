...
// node的主循环
// ywl 输入：propc,recvc,confc,tickc,advancec
// ywl 输出：readyc
func (n *node) run(r *raft) {
	...
	for {
		...
		case <-n.tickc: // ywl: 读取外部定时发过来的信号
			r.tick() // ywl: 调用的是  raft.tickElection() 或  raft.tickHeartbeat()
		case readyc <- rd:
			// 通过channel写入ready数据
...
// 应用配置更新,返回当前配置状态
// ywl: 输入到 confc ，输出从 confstatec 取出来。【同步等待】
